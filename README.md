...
### Baseline Models
We initially select three forgery detection methods—UnivFD, DualNet, and SPSL—for evaluation. Since these methods lack localization capabilities, we restricted our assessment to their binary detection performance. Subsequently, we select the face forgery localization method MSCCNet and the conventional image manipulation localization models SparseViT, IML-ViT, and Mesorch. MSCCNet and IML-ViT has a classification head that can simultaneously output image classification and forgery localization results, while SparseViT and Mesorch lack the classification head and can only output forgery localization results. In this study, we take the average of the forgery masks predicted by SparseViT and Mesorch as the image classification results, following the setting of MVSS-Net.
